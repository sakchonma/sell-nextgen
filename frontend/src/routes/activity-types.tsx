...
function ActivityTypesPage() {
  ...
  const { types, loading, error, reload } = useActivityTypes(undefined, canManage);
    ...
    return (
      <div className="p-4 md:p-6 space-y-6 max-w-5xl">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">ดูรายการประเภทที่ใช้ในระบบ · แก้ไขได้เฉพาะ Manager+</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-800 divide-y divide-slate-800">
          {loading && <div className="px-4 py-6 text-sm text-slate-500">กำลังโหลด...</div>}
          {!loading && sortedTypes.map(item => (
            <div key={item._id} className="px-4 py-3">
              <div className="text-sm font-semibold text-slate-100">{item.labelTh || item.label}</div>
              <div className="text-[11px] text-slate-500 mt-0.5">{item.code} · {item.scopes.join(', ')}</div>
            </div>
          ))}
        </div>
      </div>
